...
def bbox_annotations_to_feature_dict(
  ...
  if image_width <= 0 or image_height <= 0:
      return feature_dict, 1
  
  if (bbox_annotations['xmax'] - bbox_annotations['xmin'] > image_width 
      or bbox_annotations['ymax'] - bbox_annotations['ymin'] > image_height):
      return feature_dict, 1
  
  
  #if len(bbox_annotations) != num_skipped:
  feature_dict = {
      'image/object/bbox/xmin': tfrecord_lib.convert_to_feature(bbox_annotations['xmin'], 'float'),
      'image/object/bbox/xmax': tfrecord_lib.convert_to_feature(bbox_annotations['xmax'], 'float'),
      'image/object/bbox/ymin': tfrecord_lib.convert_to_feature(bbox_annotations['ymin'], 'float'),
      'image/object/bbox/ymax': tfrecord_lib.convert_to_feature(bbox_annotations['ymax'], 'float'),
      'image/object/class/text': tfrecord_lib.convert_to_feature(
          bbox_annotations['category_names'].encode('utf8')
      ),
      'image/object/class/label': tfrecord_lib.convert_to_feature(
          bbox_annotations['category_id']
      ),
      'image/object/is_crowd': tfrecord_lib.convert_to_feature(
          bbox_annotations['is_crowd']
      ),
      'image/object/area': tfrecord_lib.convert_to_feature(
          bbox_annotations['area'], 'float'
      ),
  }
...
def create_tf_example(image,
  ...
  feature_dict["image/label"] = tfrecord_lib.convert_to_feature(label, 'int64') # may need to update for object detection
...
def _load_object_annotations(object_annotations_file):
  ...
  for annotation in obj_annotations['annotations']:
    ...
    img_to_obj_annotation[image_id] = annotation
